...
- SQL-скрипт для Supabase: `supabase-schema.sql`.
...
1. Создайте **Static Site** на Render и подключите репозиторий.
2. Build Command: оставьте пустым.
3. Publish Directory: `.`
4. После деплоя проверьте, что `SUPABASE_URL` и `SUPABASE_ANON_KEY` подставлены в `index.html`.
